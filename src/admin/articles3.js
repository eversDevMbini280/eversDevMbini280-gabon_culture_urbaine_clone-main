...
const Articles3 = ({ apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000' }) => {
  ...
  // ─── fetchData wrapped in useCallback to satisfy react-hooks/exhaustive-deps ───
  const fetchData = useCallback(async (token) => {
    ...
    try {
      ...
      const [storiesData, resourcesData, programmesData, categoriesData, sectionsData] = await Promise.all([
        safeJson(storiesResponse),
        safeJson(resourcesResponse),
        safeJson(programmesResponse),
        safeJson(categoriesResponse),
        safeJson(sectionsResponse),
      ]);

      // If categories/sections are still empty, retry without auth header.
      let resolvedCategories = categoriesData;
      let resolvedSections = sectionsData;
      if (!resolvedCategories.length) {
        const retryCategories = await fetch(`${apiUrl}/api/categories/`, { cache: 'no-store' }).then(safeJson).catch(() => []);
        if (retryCategories.length) resolvedCategories = retryCategories;
      }
      if (!resolvedSections.length) {
        const retrySections = await fetch(`${apiUrl}/api/sections/`, { cache: 'no-store' }).then(safeJson).catch(() => []);
        if (retrySections.length) resolvedSections = retrySections;
      }
      ...
      setSuccessStories(Array.isArray(storiesData) ? storiesData.map(normalizeImageUrl) : []);
      setResources(Array.isArray(resourcesData) ? resourcesData.map(normalizeImageUrl) : []);
      setProgrammes(Array.isArray(programmesData) ? programmesData.map(normalizeImageUrl) : []);
      setCategories(resolvedCategories);
      setSections(resolvedSections);

      const section = resolvedSections.find((s) =>
        s.slug === (activeTab === 'success-stories' ? 'success-stories' : activeTab === 'resources' ? 'resources' : 'programmes-de-soutien')
      );
      const defaultCategoryId =
        (activeTab === 'success-stories' ? resolvedCategories.find((c) => (c.name || '').toLowerCase().includes('success'))?.id : null) ||
        (activeTab === 'resources' ? resolvedCategories.find((c) => (c.name || '').toLowerCase().includes('resource'))?.id : null) ||
  ...
  // ─── useEffect now includes fetchData in deps ───────────────────────────────
  useEffect(() => {
  ...
  // ─── Thumbnail with onError fallback: kept as <img> with eslint-disable ──────
              ...
              {filteredData().map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap relative">
                    {/* ─── Fix: eslint-disable for <img> needed for onError fallback ─── */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="mt-1 flex items-center">
                ...
                {imagePreview && (
                  <div className="ml-4">
                    {/* ─── Fix: next/image for form preview ─── */}
                    <Image
                      src={imagePreview}
                      alt="Preview"
                      width={128}
