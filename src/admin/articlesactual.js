...
const fetchActualites = async () => {
    ...
    try {
      ...
      setError(error.message || "Failed to fetch actualites. Please try again later.");
  ...
  useEffect(() => {
    fetchActualites();
    const intervalId = setInterval(fetchActualites, 24 * 60 * 60 * 1000); // Refresh daily
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(`Error ${isEditModalOpen ? 'updating' : 'adding'} actualite:`, error);
      setFormError(error.message || `Failed to ${isEditModalOpen ? 'update' : 'add'} actualite. Please try again.`);
    }
  };
  ...
  const handleDeleteActualite = async () => {
    ...
    try {
      ...
      setError(error.message || 'Failed to delete actualite. Please try again.');
      setIsDeleteModalOpen(false);
    }
  };
...
//       const newActualite = await response.json();
//       // Update description with backend URLs if image or video was uploaded
//       let updatedDescription = formData.description;
//       if (newActualite.image_url && imagePreview?.startsWith('blob:')) {
//         updatedDescription = updatedDescription.replaceAll(imagePreview, newActualite.image_url);
//       }
//       if (newActualite.video_url && videoPreview?.startsWith('blob:')) {
//         updatedDescription = updatedDescription.replaceAll(videoPreview, newActualite.video_url);
//       }

//       await fetchActualites();
//       setFormSuccess(`Actualite ${isEditModalOpen ? 'updated' : 'added'} successfully!`);
//       setFormData({ title: '', description: '', status: 'published', image: null, video: null });
...
//     setVideoPreview(actualite.video_url || null);
//     setIsEditModalOpen(true);
//   };
