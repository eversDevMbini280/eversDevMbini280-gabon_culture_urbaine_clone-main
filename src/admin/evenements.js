...
const GestionDesEvenements = ({
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setErrorMessage(
          "Échec de la récupération des utilisateurs. Veuillez réessayer.",
        );
      }
    };
    ...
    const fetchCategories = async () => {
      try {
        ...
        setErrorMessage(
          "Échec de la récupération des catégories. Veuillez réessayer.",
        );
      }
    };
  ...
  const filteredEvents = events.filter((event) => {
    ...
    const matchesCategory =
      activeCategory === "all" ||
      event.category?.id === parseInt(activeCategory);
  ...
  const toggleFeatured = async (id) => {
    try {
      ...
      setErrorMessage(
        "Échec de la mise à jour du statut \"À la une\". Veuillez réessayer.",
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isNaN(organizerId) || isNaN(categoryId)) {
        throw new Error(
          "Les identifiants de l'organisateur et de la catégorie doivent être valides",
        );
      }
      ...
      formDataToSend.append("date", `${formData.date}T00:00:00Z`);
      if (formData.end_date?.trim()) {
        formDataToSend.append("end_date", `${formData.end_date}T00:00:00Z`);
      }
      ...
      formDataToSend.append(
        "attendees",
        (parseInt(formData.attendees) || 0).toString(),
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.detail || "Échec de la création de l'événement",
        );
      }
      ...
      setErrorMessage(
        error.message ||
          "Échec de la création de l'événement. Veuillez réessayer.",
      );
    } finally {
    ...
    <>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Gérez tous les événements, concerts, festivals et autres activités
            culturelles
          </p>
        </div>
        <button
      ...
      <div className="bg-white p-4 rounded-lg shadow mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          {/* Filtre catégorie — utilise categories dynamiques */}
          <select
            className="pl-3 pr-10 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:text-white dark:border-gray-600"
            value={activeCategory}
            onChange={(e) => setActiveCategory(e.target.value)}
            aria-label="Filtrer par catégorie"
          >
            <option value="all" className="text-gray-900 bg-white dark:text-white dark:bg-gray-800">
              Tous
            </option>
      ...
      <div className="bg-white rounded-lg shadow mb-6 max-h-96 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="p-4 space-y-4">
            {filteredEvents.map((event) => (
              ...
              >
                <div className="flex-shrink-0 h-10 w-10 rounded bg-gray-100 flex items-center justify-center border">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      {categories.find((c) => c.id === event.category?.id)
          ...
          <div className="p-12 flex flex-col items-center justify-center text-gray-500">
            ...
            <p className="text-sm text-gray-400">
              {searchQuery || activeCategory !== "all" || activeFilter !== "all"
                ...
                : "Ajoutez des événements en cliquant sur le bouton ci-dessus"}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Remplissez le formulaire ci-dessous pour ajouter un nouvel événement
          </p>
        </div>
        <div className="p-6">
          {errorMessage && (
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        tickets_available: e.target.checked,
                      })
                    }
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                  <label
                    htmlFor="tickets_available"
                    className="ml-2 block text-sm text-gray-700"
                  >
                    Billets disponibles
                  </label>
            ...
            <div>
              ...
              <textarea
                ...
                style={{ color: "black" }}
                rows={4}
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500 resize-vertical"
