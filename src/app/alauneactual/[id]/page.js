...
import { ArrowLeft, Calendar, User, Tag, Share2, Play, Volume2, VolumeX, ChevronLeft, ChevronRight } from 'lucide-react'
import Link from 'next/link'
import { useParams, useRouter } from 'next/navigation'
import Footer from '@/components/Footer'
import Head from 'next/head'

const AlauneactualArticlePage = () => {
  ...
  const RelatedArticle = memo(({ article, apiUrl, handleImageError, onVideoHover }) => {
    return (
      ...
      >
        <Link href={`/alauneactual/${article.id}`} className="block group">
          <div className="relative rounded-xl overflow-hidden aspect-[2/3]">
            {article.video_url ? (
              <>
                <img
                  src={article.image_url || `${apiUrl}/api/placeholder/250/375?text=${encodeURIComponent(article.title || 'Article')}&color=429261`}
                  alt={article.title}
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch main article
      const articleRes = await fetch(`${API_BASE_URL}/api/articles/${id}`)
      if (!articleRes.ok) {
      ...
      if (imageUrl && !imageUrl.startsWith('http') && !imageUrl.startsWith('//')) {
        ...
        imageUrl = `${API_BASE_URL}/api/placeholder/1200/720?text=${encodeURIComponent(articleData.title || 'Article')}&color=429261`
      }
      ...
      setArticle({
        ...
        category: articleData.category?.name || 'Uncategorized',
      ...
      const relatedRes = await fetch(`${API_BASE_URL}/api/articles/alauneactual?status=published&limit=6&exclude=${id}`)
      if (!relatedRes.ok) {
        ...
        const processedRelated = relatedData.map((rel) => {
          ...
          if (rImageUrl && !rImageUrl.startsWith('http') && !rImageUrl.startsWith('//')) {
            ...
            rImageUrl = `${API_BASE_URL}/api/placeholder/250/375?text=${encodeURIComponent(rel.title || 'Article')}&color=429261`
          }
  ...
  const handleImageError = useCallback(
    (e, articleId, placeholderText, size = { width: 800, height: 400 }, color = '429261') => {
      const src = e.target.src
      if (loadedImages.current.has(src)) {
        console.log(`Image already processed for article ${articleId}: ${src}`)
        return
      }
      loadedImages.current.add(src)
      console.warn(`Image failed to load for article ${articleId}: ${src}`)
      const { width, height } = size
      e.target.src = `${API_BASE_URL}/api/placeholder/${width}/${height}?text=${encodeURIComponent(placeholderText || 'Article')}&color=${color}`
        ...
        <div className="relative h-[60vh] w-full">
          {article?.video_url && !previewVideoError ? (
            ...
            <div className="relative w-full h-full" onClick={article.video_url ? handleVideoPlay : undefined}>
              <img
                src={article.image_url || `${API_BASE_URL}/api/placeholder/1200/720?text=${encodeURIComponent(article.title || 'Article')}&color=429261`}
                alt={article.title}
                className="w-full h-full object-cover"
                onError={(e) => handleImageError(e, article.id, article.title, { width: 1200, height: 720 })}
                loading="eager"
              />
              {article.video_url && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/20 hover:bg-black/30 transition-colors z-10">
        ...
        <div className="max-w-4xl mx-auto px-4 -mt-20 relative z-10">
          <article className="bg-white rounded-xl shadow-xl p-6 md:p-8 mb-8">
            ...
            <div className="mb-8">
              ...
              >
                <div
                  className="prose max-w-none prose-img:rounded-xl text-black prose-p:text-base prose-p:leading-relaxed prose-h1:text-[#429261] prose-h2:text-[#429261] prose-h3:text-[#347048]"
                  style={{
                ...
                {relatedArticles.length > (isMobile ? 1 : 3) && (
                  <div className="flex justify-center mt-4 gap-2">
                    <button
                      ...
                      className="p-2 bg-[#429261] text-white rounded-full disabled:opacity-50"
                    >
...
export default AlauneactualArticlePage