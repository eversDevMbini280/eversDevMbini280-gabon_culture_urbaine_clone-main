...
import { ArrowLeft, Calendar, User, Tag, Share2, ChevronLeft, ChevronRight, Play, Volume2, VolumeX, Star } from 'lucide-react';
...
const RelatedItem = memo(({ item, apiUrl, handleImageError, onVideoHover, type }) => {
  return (
    <Link href={`/art-culinaire/${type === 'recipe' ? 'recettes' : 'chefs'}/${item.id}`}>
      <div
        ...
        <div className="rounded-xl overflow-hidden aspect-[2/3] relative">
          {item.video_url ? (
            <div className="relative w-full h-full">
              <img
                src={item.image || `${apiUrl}/api/placeholder/250/375?text=${encodeURIComponent(item.name || 'Item')}&color=2563eb`}
                alt={item.name}
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                onError={(e) => handleImageError(e, item.id, item.name, { width: 250, height: 375 })}
                loading="lazy"
              />
              <video
          ...
          <div className="absolute bottom-4 left-4">
            <span className="bg-blue-600 text-white text-xs font-medium px-2 py-1 rounded">{type}</span>
          </div>
        </div>
        <h4 className="mt-3 font-medium text-blue-600 group-hover:text-blue-800 transition line-clamp-2">{item.name}</h4>
        <p className="text-sm text-gray-500">{item.date || 'N/A'}</p>
      </div>
    </Link>
  );
});

const ChefArticlePage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (id) {
      const getChefData = async () => {
        ...
        setError(null);
        try {
          // Fetch main chef article
          const chefResponse = await fetch(`${apiUrl}/api/arts-traditions-articles/${id}`);
          if (!chefResponse.ok) {
            throw new Error('Chef profile not found');
          }
          const chefData = await chefResponse.json();

          // Verify arts_traditions_type is 'chef'
          if (chefData.arts_traditions_type !== 'chef') {
            throw new Error('Article is not a chef profile');
          }

          // Normalize image_url
          let imageUrl = chefData.image_url;
          if (imageUrl && !imageUrl.startsWith('http') && !imageUrl.startsWith('//')) {
            imageUrl = `${apiUrl}${imageUrl.startsWith('/') ? imageUrl : `/${imageUrl}`}`;
          } else if (!imageUrl) {
            imageUrl = `${apiUrl}/api/placeholder/1200/600?text=${encodeURIComponent(chefData.title || 'Chef')}&color=2563eb`;
          }

          // Normalize video_url
          let videoUrl = chefData.video_url ?? null; // Explicitly handle null/undefined
          ...
          setChef({
            ...
            image: imageUrl,
            video_url: videoUrl,
            type: chefData.arts_traditions_type,
            author: chefData.author_name || 'Anonyme',
            date: chefData.date
              ? new Date(chefData.date).toLocaleDateString('fr-FR', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                })
              : new Date(chefData.created_at).toLocaleDateString('fr-FR', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                }),
            content: chefData.content,
            specialty: chefData.specialty,
            recipes_count: chefData.recipes_count,
            rating: chefData.rating,
            reviews: chefData.reviews,
          });

          // Fetch related items (other chefs or recipes)
          const relatedResponse = await fetch(
            `${apiUrl}/api/arts-traditions-articles/?arts_traditions_type=chef,recipe&exclude=${id}&limit=6`
          );
          ...
          const processedRelated = relatedData.map((item) => {
            ...
            if (relatedImageUrl && !relatedImageUrl.startsWith('http') && !relatedImageUrl.startsWith('//')) {
              ...
              relatedImageUrl = `${apiUrl}/api/placeholder/250/375?text=${encodeURIComponent(item.title || 'Item')}&color=2563eb`;
            }

            let relatedVideoUrl = item.video_url ?? null; // Explicitly handle null/undefined
            ...
            return {
              ...
              video_url: relatedVideoUrl,
              type: item.arts_traditions_type,
              date: item.date
  ...
  // Reset video state when chef changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (videoRef.current && chef?.video_url) {
      const video = videoRef.current;

      const handleError = () => {
        console.error('Main video error:', video.error?.message || 'Unknown error');
  ...
  }, [chef?.video_url, isVideoLoading]);

  // Preview video event listeners
  useEffect(() => {
    if (previewVideoRef.current && chef?.video_url && !previewVideoError) {
      const video = previewVideoRef.current;

      const handleError = () => {
        console.error('Preview video error:', video.error?.message || 'Unknown error');
  ...
  }, [chef?.video_url, previewVideoError]);
  ...
  const handleImageError = useCallback(
    (e, itemId, placeholderText, size = { width: 800, height: 300 }, color = '2563eb') => {
      const src = e.target.src;
      if (!loadedImages.current.has(src)) {
        console.warn(`Image failed to load for item ${itemId}:`, src);
        loadedImages.current.add(src);
        const { width, height } = size;
        e.target.src = `${apiUrl}/api/placeholder/${width}/${height}?text=${encodeURIComponent(
  ...
  const handleVideoPlay = useCallback(() => {
    if (!videoRef.current || !chef?.video_url) {
      ...
      setVideoError('Aucune vidéo disponible');
      return;
    }
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: chef?.title,
          text: `Découvrez le profil de ${chef?.title}`,
  ...
  // Carousel navigation
  const maxPossibleSlide = relatedItems.length - (isMobile ? 1 : 3);
      ...
      <div className="min-h-screen bg-blue-900 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-xl text-center">
          ...
          <p className="text-gray-600 mb-6">
            {error || "Le profil du chef que vous recherchez n'existe pas ou a été déplacé."}
          </p>
          <button
          onClick={handleBack}
  ...
  return (
    <>
      <Head>
        <title>{chef.title} | Publiteur Art Culinaire</title>
        <meta name="description" content={chef.content.substring(0, 160)} />
        <meta property="og:title" content={chef.title} />
        <meta property="og:image" content={chef.image} />
        <meta property="og:description" content={chef.content.substring(0, 160)} />
      ...
      >
        ...
        <div className="relative h-[50vh] w-full">
          {chef.video_url && !previewVideoError ? (
            <div className="relative w-full h-full">
              <div className="relative w-full h-full" onClick={handleVideoPlay}>
                <video
                  ...
                  poster={chef.image}
                >
                  <source src={chef.video_url} type="video/mp4" />
                  <source src={chef.video_url.replace('.mp4', '.webm')} type="video/webm" />
                </video>
            ...
            <div className="relative w-full h-full">
              <div className="relative w-full h-full" onClick={chef.video_url ? handleVideoPlay : undefined}>
                <img
                  src={chef.image}
                  alt={chef.title}
                  className="w-full h-full object-cover"
                  onError={(e) => handleImageError(e, chef.id, chef.title, { width: 1200, height: 600 })}
                  loading="eager"
                />
                {chef.video_url && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/20 hover:bg-black/30 transition-colors z-10">
          ...
          <div className="absolute top-4 right-4 z-10 flex gap-2">
            ...
            <span className="bg-blue-600 text-white text-sm font-medium px-3 py-1 rounded-full">Chef</span>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto -mt-20 relative z-10 px-4">
          <motion.div
            ...
            transition={{ duration: 0.5 }}
          >
            <h1 className="text-2xl md:text-3xl font-bold text-blue-600 mb-4">{chef.title}</h1>
            <div className="flex flex-wrap items-center text-gray-500 mb-6 gap-y-2">
              <div className="flex items-center mr-6">
                <User className="w-4 h-4 mr-2" />
                <span>{chef.author}</span>
              </div>
              <div className="flex items-center mr-6">
              ...
              {chef.rating && (
                <div className="flex items-center mr-6">
                  ...
                  <span>
                    {chef.rating} ({chef.reviews || 0} avis)
                  </span>
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-4 items-center">
                {chef.specialty && (
                  <div className="flex items-center">
                    <Tag className="w-4 h-4 mr-2" />
                    <span>Spécialité: {chef.specialty}</span>
                  </div>
                )}
                {chef.recipes_count && (
                  <div className="flex items-center">
                    <Tag className="w-4 h-4 mr-2" />
                    <span>Recettes: {chef.recipes_count}</span>
                  </div>
                )}
              </div>
            </div>
            {/* Scrollable Chef Content */}
            <div className="mb-8">
              <div
              ...
              >
                <div
                  className="prose max-w-none prose-img:rounded-xl text-black prose-p:text-base prose-p:leading-relaxed prose-h1:text-blue-600 prose-h2:text-blue-800 prose-h3:text-blue-600"
                  style={{
                ...
                >
                  <div
                    className="chef-content text-black leading-relaxed"
                    style={{ color: '#000000', lineHeight: '1.7' }}
                    dangerouslySetInnerHTML={{ __html: chef.content }}
                  />
                </div>
              </div>
          ...
          >
            ...
            >
              ...
              <div className="relative w-full bg-black rounded-lg overflow-hidden flex justify-center">
                {videoError ? (
                  <div className="w-full h-full flex flex-col items-center bg-gray-800 text-white text-center p-6">
                            ...
                            if (videoRef.current) {
                              if (videoPlaying) {
                                ...
                                videoRef.current.play().then(() => {
                                  ...
                                  setVideoError('Impossible de jouer la vidéo.');
                                });
                              }
                            }
                          }}
                          className="bg-white bg-opacity-20 hover:bg-blue-600 rounded-full p-3 md:p-2 transition"
                          aria-label={videoPlaying ? 'Pause' : 'Jouer'}
                        >
                          {videoPlaying ? (
                            <svg
...
export default ChefArticlePage;