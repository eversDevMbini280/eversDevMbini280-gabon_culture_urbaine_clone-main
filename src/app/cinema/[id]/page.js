...
const RelatedArticle = memo(({ article, apiUrl, handleImageError, onVideoHover }) => {
  return (
    <Link href={`/cinema/${article.id}`} key={article.id}>
      <div
        ...
        <div className="rounded-xl overflow-hidden aspect-[2/3] relative">
          {article.video_url ? (
            <div className="relative w-full h-full">
              <img
                src={article.image_url || `${apiUrl}/api/placeholder/250/375?text=${encodeURIComponent(article.title || 'Article')}&color=8B0000`}
                alt={article.title}
          ...
          <div className="absolute bottom-4 left-4">
            <span className="bg-[#8B0000] text-white text-xs font-medium px-2 py-1 rounded">{article.category}</span>
          </div>
        </div>
        <h4 className="mt-3 font-medium text-[#8B0000] group-hover:text-[#6d0000] transition line-clamp-2">{article.title}</h4>
...
const CinemaArticlePage = () => {
    ...
    if (id) {
      const getArticleData = async () => {
        ...
        try {
          ...
          // Ensure the article is a cinema article
          if (!articleData.is_cinema) {
            throw new Error('Article is not a cinema article');
          }
          ...
          if (imageUrl && !imageUrl.startsWith('http') && !imageUrl.startsWith('//')) {
            ...
            imageUrl = `${apiUrl}/api/placeholder/1200/600?text=${encodeURIComponent(articleData.title || 'Article')}&color=8B0000`;
          }
          ...
          setArticle({
            ...
            category: articleData.category?.name || 'Cinéma',
          ...
          // Fetch related cinema articles
          const relatedResponse = await fetch(`${apiUrl}/api/articles/cinema?status=published&exclude=${id}`);
          ...
          const related = relatedData.map((item) => {
            ...
            if (relatedImageUrl && !relatedImageUrl.startsWith('http') && !relatedImageUrl.startsWith('//')) {
              ...
              relatedImageUrl = `${apiUrl}/api/placeholder/250/375?text=${encodeURIComponent(item.title || 'Article')}&color=8B0000`;
            }
            ...
            return {
              ...
              category: item.category?.name || 'Cinéma',
  ...
  // Image error handler
  const handleImageError = useCallback((e, itemId, placeholderText, size = { width: 800, height: 300 }, color = '8B0000') => {
    ...
    return (
      <div className="min-h-screen bg-[#8B0000] flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-xl text-center">
          <h1 className="text-2xl font-bold text-[#8B0000] mb-4">Erreur</h1>
          <p className="text-gray-600 mb-6">{error || "L'article que vous recherchez n'existe pas ou a été déplacé."}</p>
          <button 
          onClick={handleBack}
           className="bg-[#8B0000] text-white px-6 py-2 rounded-lg hover:bg-[#6d0000] transition">
      ...
      <div className="min-h-screen bg-[#8B0000]">
        ...
        <div className="relative h-[50vh] w-full">
          ...
          <div className="absolute top-4 right-4 z-10 flex gap-2">
            ...
            <span className="bg-[#8B0000] text-white text-sm font-medium px-3 py-1 rounded-full">{article.category}</span>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto -mt-20 relative z-10 px-4">
          <div className="bg-white rounded-xl shadow-xl p-6 md:p-8">
            <h1 className="text-2xl md:text-3xl font-bold text-[#8B0000] mb-4">{article.title}</h1>
            <div className="flex flex-wrap items-center text-gray-500 mb-6 gap-y-2">
              ...
              <div className="ml-auto">
                <button
                  onClick={handleShare}
                  className="flex items-center text-[#8B0000] hover:text-[#6d0000]"
                  aria-label="Partager l'article"
                >
            ...
            <div className="mb-8">
              ...
              >
                <div
                  className="prose max-w-none prose-img:rounded-xl text-black prose-p:text-base prose-p:leading-relaxed prose-h1:text-[#8B0000] prose-h2:text-[#6d0000] prose-h3:text-[#8B0000]"
                  style={{
            ...
            {relatedArticles.length > 0 && (
              <div className="mt-12">
                <h3 className="text-xl font-bold text-[#8B0000] mb-6 flex items-center">
                  <div className="w-2 h-8 bg-[#a52a2a] mr-3 rounded-full"></div>
                  Articles similaires
                </h3>
                <div className="flex gap-2 mb-4">
                  <button
                    onClick={handlePrev}
                    className="p-2 bg-[#8B0000] rounded-lg hover:bg-[#6d0000] transition-colors disabled:opacity-50"
                    ...
                    {isMobile && (
                      <div className="min-w-[25vw] flex items-center justify-start pr-4">
                        <div className="w-8 h-full flex items-center justify-center">
                          <ChevronRight className="w-6 h-6 text-[#8B0000] animate-pulse" />
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {videoError ? (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-gray-800 text-white text-center p-6">
                    ...
                    <button
                      ...
                      className="bg-[#8B0000] hover:bg-[#6d0000] text-white px-6 py-3 rounded-lg transition text-lg md:text-base"
                    >
                      Try Again
                    </button>
                  </div>
                      ...
                      <div className="flex items-center justify-between mt-4 md:mt-2">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-3 md:gap-2 px-4 md:px-3 py-2 md:py-1 rounded-full transition ${
                            isMuted ? 'bg-red-500 hover:bg-red-600' : 'bg-[#8B0000] hover:bg-[#6d0000]'
                          }`}
...
export default CinemaArticlePage;