...
const RelatedArticle = memo(({ article, apiUrl, handleImageError, onVideoHover }) => {
  return (
    <Link href={`/decouverte/${article.id}`} key={article.id}>
      <div
        ...
        <div className="rounded-xl overflow-hidden aspect-[2/3] relative">
          {article.video_url ? (
            <div className="relative w-full h-full">
              <img
                src={article.image_url || `${apiUrl}/api/placeholder/250/375?text=${encodeURIComponent(article.title || 'Article')}&color=047857`}
                alt={article.title}
          ...
          <div className="absolute bottom-4 left-4">
            <span className="bg-[#047857] text-white text-xs font-medium px-2 py-1 rounded">{article.section}</span>
          </div>
        </div>
        <h4 className="mt-3 font-medium text-[#047857] group-hover:text-[#065f46] transition line-clamp-2">{article.title}</h4>
...
const DecouverteArticlePage = () => {
  ...
  useEffect(() => {
    if (id) {
      const getArticleData = async () => {
        ...
        try {
          // Fetch main article
          const articleResponse = await fetch(`${apiUrl}/api/arts-traditions-articles/${id}?status=published`);
          ...
          setArticle({
            ...
            category: articleData.category?.name || 'Découverte',
          ...
          // Fetch related arts & traditions articles
          const relatedResponse = await fetch(`${apiUrl}/api/arts-traditions-articles/?status=published&exclude=${id}`);
          ...
          const processedRelated = relatedData.map((item) => {
            ...
            if (relatedImageUrl && !relatedImageUrl.startsWith('http') && !relatedImageUrl.startsWith('//')) {
              ...
              relatedImageUrl = `${apiUrl}/api/placeholder/250/375?text=${encodeURIComponent(item.title || 'Article')}&color=047857`;
            }
            ...
            return {
              ...
              category: item.category?.name || 'Découverte',
              section: item.section?.name || 'Arts & Traditions',
  ...
  // Image error handler with Base64 fallback
  const handleImageError = useCallback((e, itemId, placeholderText, size = { width: 800, height: 300 }, color = '047857') => {
    const src = e.target.src;
    if (!loadedImages.current.has(src)) {
      console.warn(`Image failed to load for item ${itemId}:`, src);
      loadedImages.current.add(src);
      const { width, height } = size;
      // Try the placeholder API first
      if (!src.includes('/api/placeholder/') && src !== FALLBACK_IMAGE) {
        ...
        // If the placeholder API fails or we're already on the placeholder, use the Base64 fallback
  ...
  return (
    <>
      <Head>
        <title>{article.title} | Gabon Découverte</title>
        ...
        <div className="max-w-4xl mx-auto -mt-20 relative z-10 px-4">
          <div className="bg-white rounded-xl shadow-xl p-6 md:p-8">
            ...
            <div className="mb-8">
              ...
              <div 
                className="max-h-96 md:max-h-[500px] overflow-y-auto overflow-x-hidden pr-4 scrollbar-thin scrollbar-thumb-[#047857] scrollbar-track-gray-100 hover:scrollbar-thumb-[#065f46]"
                style={{
                  scrollbarWidth: 'thin',
                  scrollbarColor: '#047857 #f3f4f6'
                }}
              >
                <div
                  className="prose max-w-none prose-img:rounded-xl text-black prose-p:text-base prose-p:leading-relaxed prose-h1:text-[#047857] prose-h2:text-[#047857] prose-h3:text-[#047857]"
                  style={{
            ...
            {relatedArticles.length > 0 && (
              <div className="mt-12">
                <h3 className="text-xl font-bold text-[#047857] mb-6 flex items-center">
                ...
                <div className="flex gap-2 mb-4">
                  <button
                    onClick={handlePrev}
                    className="p-2 bg-[#047857] rounded-lg hover:bg-[#065f46] transition-colors disabled:opacity-50"
                    ...
                    {isMobile && (
                      <div className="min-w-[25vw] flex items-center justify-start pr-4">
                        <div className="w-8 h-full flex items-center justify-center">
                          <ChevronRight className="w-6 h-6 text-[#047857] animate-pulse" />
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {videoError ? (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-gray-800 text-white text-center p-6">
                    ...
                    <button
                      ...
                      className="bg-[#047857] hover:bg-[#065f46] text-white px-6 py-3 rounded-lg transition text-lg md:text-base"
                    >
                      Try Again
                    </button>
                  </div>
                      ...
                      <div className="flex items-center justify-between mt-4 md:mt-2">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-3 md:gap-2 px-4 md:px-3 py-2 md:py-1 rounded-full transition ${
                            isMuted ? 'bg-red-500 hover:bg-red-600' : 'bg-[#047857] hover:bg-[#065f46]'
                          }`}
...
export default DecouverteArticlePage;