...
const RelatedArticle = memo(({ article, apiUrl, handleImageError, onVideoHover }) => {
  return (
    ...
    >
      <Link href={`/eventactual/${article.id}`} className="block group">
        <div className="relative rounded-xl overflow-hidden aspect-[3/4]">
          {article.video_url ? (
            <div className="relative w-full h-full">
              <img
                src={article.image_url || `${apiUrl}/api/placeholder/300/400?text=${encodeURIComponent(article.title || 'Article')}&color=429261`}
                alt={article.title}
...
const EventualArticlePage = () => {
  ...
  // Fetch article
  const fetchEventualArticle = async (articleId) => {
    try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch main article
        const articleData = await fetchEventualArticle(id)
        if (!articleData) {
        ...
        const relatedResponse = await fetch(`${API_BASE_URL}/api/articles/eventactual?status=published&limit=3`)
  ...
  // Image error handler
  const handleImageError = useCallback((e, articleId, title) => {
    ...
    if (!loadedImages.current.has(src)) {
      ...
      e.target.src = `${API_BASE_URL}/api/placeholder/300/400?text=${encodeURIComponent(title || 'Article')}&color=429261`
    ...
    <div className="min-h-screen bg-[#429261]">
      ...
      <div className="relative h-[60vh] w-full">
        ...
        <div className="absolute top-4 right-4 z-10 flex gap-2">
          ...
          <span className="bg-[#429261] text-white text-sm font-medium px-3 py-1 rounded-full">
            {article.category}
          </span>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-20 relative z-10">
        <article className="bg-white rounded-xl shadow-xl p-6 md:p-8 mb-8">
          ...
          <div className="mb-8">
            <h2 className="text-xl font-bold text-[#429261] mb-4 flex items-center">
              <div className="w-2 h-6 bg-[#7dbb8e] mr-3 rounded-full"></div>Contenu
            </h2>
            <div 
              className="max-h-96 md:max-h-[500px] overflow-y-auto overflow-x-hidden pr-4 scrollbar-thin scrollbar-thumb-[#429261] scrollbar-track-gray-100 hover:scrollbar-thumb-[#347048]"
              style={{
                scrollbarWidth: 'thin',
                scrollbarColor: '#429261 #f3f4f6'
              }}
            >
              <div
                className="prose max-w-none prose-lg prose-img:rounded-xl text-black prose-p:text-base prose-p:leading-relaxed prose-h1:text-[#429261] prose-h2:text-[#429261] prose-h3:text-[#429261]"
                style={{
                ...
                {isMobile && (
                  <div className="min-w-[25vw] flex items-center justify-start pr-4">
                    <div className="w-8 h-full flex items-center justify-center">
                      <ChevronRight className="w-6 h-6 text-[#429261] animate-pulse" />
                    </div>
                  </div>
                )}
              </div>
...
export default EventualArticlePage