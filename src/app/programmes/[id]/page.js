...
const ProgrammeDetailPage = () => {
  ...
  // Fetch programme and related data
useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch programme data without token
      const programmeRes = await fetch(`${API_BASE_URL}/api/programmes/${id}`, {
        cache: 'no-store',
        headers: {
      ...
      if (!programmeRes.ok) {
        const errorData = await programmeRes.json().catch(() => ({}));
        throw new Error(errorData.detail || `Échec de la récupération du programme (Status: ${programmeRes.status})`);
      }
      const programmeData = await programmeRes.json();

      let imageUrl = programmeData.image_url;
      if (imageUrl && !imageUrl.startsWith("http") && !imageUrl.startsWith("//")) {
        imageUrl = `${API_BASE_URL}${imageUrl.startsWith("/") ? imageUrl : `/${imageUrl}`}`;
      }

      let videoUrl = programmeData.video_url;
      ...
      setProgramme({
        ...
        author_name: programmeData.author_name || 'Anonyme',
        category: programmeData.category || { name: 'Programmes de Soutien' },
        title: programmeData.title || 'Programme sans titre',
        content: programmeData.content || 'Aucun contenu disponible',
        created_at: programmeData.created_at || null,
      });

      // Fetch related programmes (also without token)
      const relatedRes = await fetch(
        `${API_BASE_URL}/api/programmes?exclude_id=${id}&limit=4`,
        {
          cache: 'no-store',
          headers: {
            Accept: 'application/json',
      ...
      if (!relatedRes.ok) {
        console.warn("Failed to fetch related programmes:", relatedRes.status);
      } else {
        const relatedData = await relatedRes.json();
        const processedRelated = relatedData
          .filter((p) => p.id !== programmeData.id)
          .map((p) => {
            let rImageUrl = p.image_url;
            ...
            return {
              ...
              category: p.category || { name: 'Programmes de Soutien' },
              title: p.title || 'Programme sans titre',
              content: p.content || 'Aucun contenu disponible',
              uniqueKey: `related-${p.id}-${Date.now()}`,
            };
          });
        setRelatedProgrammes(processedRelated);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (videoRef.current && programme?.video_url) {
  ...
  }, [programme?.video_url, isVideoLoading]);

  // Preview video event listeners
  useEffect(() => {
    if (previewVideoRef.current && programme?.video_url && !previewVideoError) {
  ...
  }, [programme?.video_url, previewVideoError]);
  ...
  const handleImageError = (e, programmeId) => {
    if (!failedImages.has(programmeId)) {
      setFailedImages((prev) => new Set(prev).add(programmeId));
  ...
  const handleVideoPlay = () => {
    if (!videoRef.current || !programme?.video_url) return;
  ...
  const handleRelatedVideoHover = (programmeId, action) => {
    const videoElement = relatedVideoRefs.current[programmeId];
    if (!videoElement) return;

    if (action === "start") {
      if (relatedVideoHover && relatedVideoHover !== programmeId) {
      ...
      setRelatedVideoHover(programmeId);
      videoElement.muted = true;
      videoElement.currentTime = 0;
      videoElement.play().catch(() => {});
    } else if (action === "stop") {
      if (relatedVideoHover === programmeId) {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: programme?.content?.substring(0, 100) + "...",
    ...
    return (
      <div className="min-h-screen bg-blue-900 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-xl text-center max-w-md">
          <h1 className="text-2xl font-bold text-blue-900 mb-4">Programme Non Trouvé</h1>
    ...
    >
      ...
      <div className="relative h-[60vh] w-full">
        {programme?.video_url && !previewVideoError ? (
          <div className="relative w-full h-full">
            <div className="relative w-full h-full" onClick={handleVideoPlay}>
              <video
                ...
                playsInline
                preload="metadata"
                poster={programme.image_url}
              >
                <source src={programme.video_url} type="video/mp4" />
                <source src={programme.video_url.replace(".mp4", ".webm")} type="video/webm" />
              </video>
          ...
          <div className="relative w-full h-full">
            <div className="relative w-full h-full" onClick={programme.video_url ? handleVideoPlay : undefined}>
              <img
                src={programme.image_url || FALLBACK_IMAGE}
                alt={programme.title}
                className="w-full h-full object-cover"
        ...
        <div className="absolute top-4 right-4 z-10 flex gap-2">
          ...
          <span className="bg-blue-900 text-white text-sm font-medium px-3 py-1 rounded-full">
            {programme.category?.name || 'Programmes de Soutien'}
          </span>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-20 relative z-10">
        ...
        >
          <h1 className="text-3xl font-bold text-blue-900 mb-4">{programme.title}</h1>
          <div className="flex flex-wrap items-center text-gray-500 mb-6 gap-y-2">
            <div className="flex items-center mr-6">
              <User className="w-4 h-4 mr-2" />
              <span>{programme.author_name}</span>
            </div>
            <div className="flex items-center mr-6">
              <Calendar className="w-4 h-4 mr-2" />
              <span>{programme.created_at ? new Date(programme.created_at).toLocaleDateString('fr-FR') : 'Date inconnue'}</span>
            </div>
            <div className="flex items-center">
              <Tag className="w-4 h-4 mr-2" />
              <span>{programme.category?.name || 'Programmes de Soutien'}</span>
            </div>
          </div>

          {/* Scrollable Programme Content */}
          <div className="mb-8">
            <div 
            ...
            <div className="relative">
              ...
              >
                {relatedProgrammes.map((related) => (
                  ...
                  >
                    <Link href={`/programmes/${related.id}`} className="block group">
              ...
              {relatedProgrammes.length > (isMobile ? 1 : 3) && (
                <div className="flex justify-center mt-4 gap-2">
                  ...
                  <button
                    onClick={() => sliderRef.current.scrollBy({ left: 300, behavior: "smooth" })}
                    disabled={currentSlide >= relatedProgrammes.length - (isMobile ? 1 : 3)}
                    className="p-2 bg-purple-900 text-white rounded-full disabled:opacity-50"
                  >
