...
const RelatedItem = memo(({ item, apiUrl, handleImageError, onVideoHover }) => {
  return (
    <Link href={`/savoirtraditionnel/${item.id}`}>
      <div
      ...
      >
        <div className="rounded-xl overflow-hidden aspect-[2/3] relative">
          ...
          <div className="absolute bottom-4 left-4">
            <span className="bg-blue-600 text-white text-xs font-medium px-2 py-1 rounded">Savoir Traditionnel</span>
          </div>
        </div>
...
const SavoirTraditionnelPage = () => {
  ...
  // Fetch savoir traditionnel data
  useEffect(() => {
    if (id) {
      const getItemData = async () => {
        ...
        setError(null);
        try {
          // Fetch main savoir traditionnel item
          const response = await fetch(`${apiUrl}/api/arts-traditions-articles/${id}`);
          if (!response.ok) {
            throw new Error('Savoir traditionnel item not found');
          }
          const data = await response.json();

          // Verify arts_traditions_type is 'savoirtraditionnel'
          if (data.arts_traditions_type !== 'savoirtraditionnel') {
            throw new Error('Article is not a savoir traditionnel item');
          }
          ...
          if (imageUrl && !imageUrl.startsWith('http') && !imageUrl.startsWith('//')) {
            ...
            imageUrl = `${apiUrl}/api/placeholder/1200/600?text=${encodeURIComponent(data.title || 'Savoir Traditionnel')}&color=2563eb`;
          }
          ...
          // Fetch related items
          const relatedResponse = await fetch(`${apiUrl}/api/arts-traditions-articles/?arts_traditions_type=savoirtraditionnel&exclude=${id}&limit=6`);
          ...
          console.error('Error fetching savoir traditionnel data:', err);
  ...
  useEffect(() => {
    if (previewVideoRef.current && item?.video_url && !previewVideoError) {
  ...
  return (
    <>
      <Head>
        <title>{item.title} | Savoir Traditionnel</title>
        ...
        <div className="relative h-[50vh] w-full">
          ...
          <div className="absolute top-4 left-4 z-10">
              ...
              onClick={handleBack}
                ...
                aria-label="Retour à Savoir Traditionnel"
              >
          ...
          <div className="absolute top-4 right-4 z-10 flex gap-2">
            ...
            <span className="bg-blue-600 text-white text-sm font-medium px-3 py-1 rounded-full">Savoir Traditionnel</span>
          </div>
        </div>
...
export default SavoirTraditionnelPage;