  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const storyRes = await fetch(`${API_BASE_URL}/api/articles/${id}`);
        if (!storyRes.ok) {
          const errorData = await storyRes.json().catch(() => ({}));
          throw new Error(errorData.detail || "Story not found");
        }
        ...
        setStory({
          ...
          author_name: storyData.author_name, // Add author_name
          author: storyData.author, // Preserve original author object
        });

        if (storyData.category_id) {
          const relatedRes = await fetch(
            `${API_BASE_URL}/api/articles/story?&status=published&category_id=${storyData.category_id}`,
          );
          if (!relatedRes.ok) {
  ...
  const handleVideoPlay = () => {
    ...
    videoRef.current
      ...
      .then(() => {
        ...
        setVideoError(null);
        setIsVideoLoading(false); // Clear loading state when playback starts
    ...
    return (
      <div className="min-h-screen bg-purple-900 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-xl text-center max-w-md">
          <h1 className="text-2xl font-bold text-purple-900 mb-4">Story Not Found</h1>
          <p className="text-gray-600 mb-6">{error}</p>
          <button
            onClick={() => router.push("/")}
            className="bg-purple-900 text-white px-6 py-2 rounded-lg hover:bg-purple-800 transition"
          >
    ...
    <div className="min-h-screen bg-purple-900">
      ...
      <div className="relative h-[60vh] w-full">
        ...
        <div className="absolute top-4 right-4 z-10 flex gap-2">
          ...
          <span className="bg-purple-900 text-white text-sm font-medium px-3 py-1 rounded-full">
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-20 relative z-10">
        <article className="bg-white rounded-xl shadow-xl p-6 md:p-8 mb-8">
          <h1 className="text-3xl font-bold text-purple-900 mb-4">{story.title}</h1>

          <div className="flex flex-wrap items-center text-gray-500 mb-6 gap-y-2">
            <div className="flex items-center mr-6">
              <User className="w-4 h-4 mr-2" />
              <span>{story.author_name || story.author?.username || "Unknown Author"}</span>
            </div>
            <div className="flex items-center mr-6">
          ...
          <div className="mb-8">
            <div 
              className="max-h-96 md:max-h-[500px] overflow-y-auto overflow-x-hidden pr-4 scrollbar-thin scrollbar-thumb-purple-300 scrollbar-track-gray-100 hover:scrollbar-thumb-purple-400"
              style={{
                scrollbarWidth: 'thin',
            ...
            >
              <div
                className="prose max-w-none prose-lg text-black prose-img:rounded-xl prose-p:text-base prose-p:leading-relaxed prose-h1:text-purple-900 prose-h2:text-purple-800 prose-h3:text-purple-700"
                style={{
            ...
            <div className="relative">
              ...
              >
                {relatedStories.map((related) => (
                  ...
                  >
                    <Link href={`/stories/${related.id}`} className="block group">
                        ...
                        {related.video_url ? (
                          <div className="relative w-full h-full">
                            <img
                              src={
                                related.image_url ||
                                `${API_BASE_URL}/api/placeholder/300/400?text=${encodeURIComponent(
                                  related.title || "Story",
                                )}&color=4f46e5`
                              }
                              alt={related.title}
